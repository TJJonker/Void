...
namespace Void::Audio {
	void FMODAudioManager::SetChannelVolume(unsigned int channelID, unsigned int volume)
	{
		...
		VOID_ASSERT(result == FMOD_OK, "Error while setting the volume of channel: '{0}' - {1}", channelID, FMOD_ErrorString(result));
	...
	void FMODAudioManager::SetChannelPitch(unsigned int channelID, float pitch)
	{
		...
		VOID_ASSERT(result == FMOD_OK, "Error while setting the pitch of channel: '{0}' - {1}", channelID, FMOD_ErrorString(result));
	...
	void FMODAudioManager::SetChannelPan(unsigned int channelID, float pan)
	{
		...
		VOID_ASSERT(result == FMOD_OK, "Error while setting the pan of channel: '{0}' - {1}", channelID, FMOD_ErrorString(result));
	...
	void FMODAudioManager::SetChannelGroupVolume(unsigned int groupID, unsigned int volume)
	{
		...
		FMOD_RESULT result = m_ChannelGroups[groupID]->Group->setVolume((float)volume);
		VOID_ASSERT(result == FMOD_OK, "Error while setting the volume of channelGroup: '{0}' - {1}", groupID, FMOD_ErrorString(result));
	...
	{
		VOID_ASSERT(groupID < m_ChannelGroups.size(), "ID out of bounds.");
		FMOD_RESULT result = m_ChannelGroups[groupID]->Group->setPitch(pitch);
		VOID_ASSERT(result == FMOD_OK, "Error while setting the pitch of channelGroup: '{0}' - {1}", groupID, FMOD_ErrorString(result));
	...
	{
		VOID_ASSERT(groupID < m_ChannelGroups.size(), "ID out of bounds.");
		FMOD_RESULT result = m_ChannelGroups[groupID]->Group->setPan(pan);
		VOID_ASSERT(result == FMOD_OK, "Error while setting the pan of channelGroup: '{0}' - {1}", groupID, FMOD_ErrorString(result));
